...
export const Header: React.FC = () => {
  ...
  const tools = [
    { name: isDanish ? 'YouTube Udskrift' : 'YouTube Transcript', href: 'https://youtubetranscript.eu/' },
  ...
  return (
    <header className={`fixed top-0 left-0 right-0 w-full z-50 transition-all duration-300 ${scrolled ? 'bg-black/80 backdrop-blur-md py-3' : 'bg-transparent py-5'}`}>
      <div className="container mx-auto px-4 md:px-6">
        <div className="flex justify-between items-center">
          ...
          <nav className="hidden md:flex items-center space-x-8">
            {navItems.map((item) => {
              ...
              if (item.isDropdown === 'tools') {
                return (
                  <div key={item.name} className="relative" ref={toolsRef}>
                    <button
              ...
              return (
                <Link
                  key={item.name}
                  to={item.href}
                  className={`text-sm font-medium transition-colors ${location.pathname === item.href ? 'text-white' : 'text-gray-200 hover:text-white'}`}
                >
                  {item.name}
                </Link>
              );
          ...
          {/* Mobile Toggle */}
          <button className="md:hidden text-white p-2" onClick={toggleMenu} aria-label={isMenuOpen ? 'Close menu' : 'Open menu'}>
